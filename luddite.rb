...
if ARGV[0] == "catchup" 
	last = client.home_timeline(:count => 1).last
	new_since = last.id
	time = Time.parse(last.created_at).localtime.strftime("%b %d %T")
	puts "catching up to #{time}"
...
if since_id
	...
	while max_id != last_max_id do 
		puts "Starting to collect messages"
		if max_id
			puts "Collecting the next batch of messages"
			...
			puts "Collecting the first batch of messages"
	...
	msgs = client.home_timeline(:count => 200)
end
